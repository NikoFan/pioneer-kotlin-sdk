...
class MavlinkConnectionNdk(ip: String, port: Int) {
    ...
    private val receiveSocket = DatagramSocket(8001)
    ...
    // === ПУБЛИЧНЫЕ МЕТОДЫ ===
    suspend fun arm(): Boolean = sendCommandWithAck(
        command = 400,
        param1 = 1.0f,
        logTag = "ARM"
    )

    suspend fun disarm(): Boolean = sendCommandWithAck(
        command = 400,
        param1 = 0.0f,
        logTag = "DISARM"
    )

    suspend fun takeoff(): Boolean = sendCommandWithAck(
        command = 22,
        logTag = "TAKEOFF"
    )

    suspend fun land(): Boolean = sendCommandWithAck(
        command = 21,
        logTag = "LAND"
    )

    fun getFlightLogs(): Array<String> = arrayOf()
    fun clearLogs() = Unit
